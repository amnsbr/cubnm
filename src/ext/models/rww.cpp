...
void rWWModel::init_constants(double dt) {
    ...
    mc.a_E = 310; // excitatory input-output parameter a (n/C)
    mc.b_E = 125; // excitatory input-output parameter b (Hz)
    mc.d_E = 0.16; // excitatory input-output parameter d (s)
    mc.a_I = 615; // inhibitory input-output parameter a (n/C)
    mc.b_I = 177; // inhibitory input-output parameter b (Hz)
    mc.d_I = 0.087; // inhibitory input-output parameter d (s)
    mc.gamma_E = (double)0.641/(double)1000.0; // excitatory kinetic parameter
    ...
    mc.dt_itau_E = mc.dt * mc.itau_E; // dt / tau_E
    mc.dt_gamma_E = mc.dt * mc.gamma_E; // dt * gamma_E
    mc.dt_itau_I = mc.dt * mc.itau_I; // dt / tau_I
    mc.dt_gamma_I = mc.dt * mc.gamma_I; // dt * gamma_I
    mc.tau_E_s = 0.1; // tau_E in seconds for FIC
    mc.tau_I_s = 0.01; // tau_I in seconds for FIC
    mc.gamma_E_s = 0.641; // gamma_E for FIC
    mc.gamma_I_s = 1.0; // gamma_I for FIC
    mc.r_I_ss = 3.9218448633; // steady-state inhibitory firing rate (Hz)
    mc.r_E_ss = 3.0773270642; // steady-state excitatory firing rate (Hz)
    mc.I_I_ss = 0.2528951325; // steady-state inhibitory current (nA)
    mc.I_E_ss = 0.3773805650; // steady-state excitatory current (nA)
    mc.S_I_ss = 0.0392184486; // steady-state inhibitory gating
    mc.S_E_ss = 0.1647572075; // steady-state excitatory gating
}
...
void rWWModel::h_step(
    ...
    _intermediate_vars[2] = noise[noise_idx] * _regional_params[3] * rWWModel::mc.sqrt_dt + rWWModel::mc.dt_gamma_E * ((1 - _state_vars[4]) * _state_vars[2]) - rWWModel::mc.dt_itau_E * _state_vars[4];
    // dS_I = noise_S_I * sigma * sqrt_dt + dt_gamma_I * r_I - dt_itau_I * S_I
    _intermediate_vars[3] = noise[noise_idx + 1] * _regional_params[3] * rWWModel::mc.sqrt_dt + rWWModel::mc.dt_gamma_I * _state_vars[3] - rWWModel::mc.dt_itau_I * _state_vars[5];
