/* 
    ...
    - Model definition: 'modelgen/recipes/jr.yaml'
...
__device__ void JRModel::step(
    ...
    _intermediate_vars[0] = 2.0 * d_JRc.nu_max / (1.0 + exp(d_JRc.r * (d_JRc.v0 - (_state_vars[1] - _state_vars[2]))));
    // sigm_y0_1  = 2.0 * nu_max / (1.0 + exp(r * (v0 - (a_1 * J * y0))))
    _intermediate_vars[1] = 2.0 * d_JRc.nu_max / (1.0 + exp(d_JRc.r * (d_JRc.v0 - (_regional_params[1] * _regional_params[0] * _state_vars[0]))));
    // sigm_y0_3  = 2.0 * nu_max / (1.0 + exp(r * (v0 - (a_3 * J * y0))))
    _intermediate_vars[2] = 2.0 * d_JRc.nu_max / (1.0 + exp(d_JRc.r * (d_JRc.v0 - (_regional_params[3] * _regional_params[0] * _state_vars[0]))));
    ...
    _intermediate_vars[6] = d_JRc.A * d_JRc.a * _intermediate_vars[0] - 2.0 * d_JRc.a * _state_vars[3] - (pow(d_JRc.a, 2)) * _state_vars[0];
    // y4_dot = A * a * (mu + a_2 * J * sigm_y0_1 + G * globalinput) - 2.0 * a * y4 - (pow(a, 2)) * y1
    _intermediate_vars[7] = d_JRc.A * d_JRc.a * (d_JRc.mu + _regional_params[2] * _regional_params[0] * _intermediate_vars[1] + _global_params[0] * tmp_globalinput) - 2.0 * d_JRc.a * _state_vars[4] - (pow(d_JRc.a, 2)) * _state_vars[1];
    // y5_dot = B * b * (a_4 * J * sigm_y0_3) - 2.0 * b * y5 - (pow(b, 2)) * y2
    _intermediate_vars[8] = d_JRc.B * d_JRc.b * (_regional_params[4] * _regional_params[0] * _intermediate_vars[2]) - 2.0 * d_JRc.b * _state_vars[5] - (pow(d_JRc.b, 2)) * _state_vars[2];
    ...
    _state_vars[3] += d_JRc.dt * _intermediate_vars[6] + d_JRc.sqrt_dt * noise[noise_idx] * _regional_params[5];
    // y4 += dt * y4_dot
    ...
    _state_vars[7] = d_JRc.cmin + (d_JRc.cmax - d_JRc.cmin) / (1.0 + exp(d_JRc.r * (d_JRc.midpoint - (_state_vars[6]))));
}